...
import vn.edu.iuh.fit.week02_lab_nguyentonganhquan_21006171_be.backend.models.OrderDetailCompositeId;
...
public class OrderDetailRepositoryImpl implements CRUDRespository<Order_Detail, OrderDetailCompositeId> {
    ...
    public boolean delete(OrderDetailCompositeId id) {
        Optional<Order_Detail> optionalOrderDetail = findById(id);
        if (optionalOrderDetail.isEmpty()) {
            return false;
        } else {
            try {
                entityManager.getTransaction().begin();
                entityManager.remove(optionalOrderDetail);
                entityManager.getTransaction().commit();
                entityManager.clear();
                return true;
            } catch (Exception e) {
    ...
    @Override
    public List<Order_Detail> findAll(Class<Order_Detail> entity) {
